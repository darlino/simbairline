...
class BookingController extends \App\Http\Controllers\Controller
{
    ...
    function saveDataImages($data): ?string
    {
        if($data){
            $png_url = "data-".random_int(9999,9999999999)."".time().".".explode('/', mime_content_type($data))[1];
            $path = public_path().'/images/data/' . $png_url;
            $i = Image::make(file_get_contents($data))->save($path);
            return "images/data/" . $png_url;
        }
    ...
    public function doCheckout(Request $request)
    {
        ...
        $booking->pay_now = (float)($booking->deposit == null ? $booking->total : $booking->deposit);
        ...
        if($booking->object_model === "space"){
            $booking->data_img_1 = $this->saveDataImages($request->input('data0'));
            $booking->data_img_2 = $this->saveDataImages($request->input('data1'));
            $booking->data_img_3 = $this->saveDataImages($request->input('data2'));
            $booking->data_img_4 = $this->saveDataImages($request->input('data3'));
        }
        ...
        if(Auth::check()) {
            try{
                $u = User::where('user_name', $user->parent);
                if($u->exists()) {
                    $com = new Commission;
                    $com->booking_id = $booking->id;
                    $com->user_id = $u->first()->id;
                    $com->unit = \App\Currency::getCurrent('currency_main');
                    $com->amount = ((float)$booking->pay_now * (float)config("app.rate"));
                    $com->commission_rate = (float)config("app.rate");
...
//        event(new VendorLogPayment($booking));
    ...
    public function setPaidAmount(Request $request){
        ...
        $id = $request->input('id');
        $remain = (float)$request->input('remain');
        ...
        $booking->paid = (float)$booking->total - $remain;
